 ...
 * GNU Lesser General Public License for more details.c'd
 *
...
static uint32_t branch_id = 0;  // for ZCash
    ...
    if (overwintered && (branch_id == 0)) {
        switch (version) {
            case 3:
                ...
                break;
            case 4:
                if (strcmp(_coin->coin_name, "Ycash") == 0) {
                    branch_id = 0x374d694f;  // YCash
                } else {
                    branch_id = 0x2bb40e60;  // Blossom
                }
                break;
        }
    }
...
void signing_txack(TransactionType *tx)
{
	...
	switch (signing_stage) {
		...
		case STAGE_REQUEST_2_PREV_META:
			...
			tx_init(&tp, tx->inputs_cnt, tx->outputs_cnt, tx->version, tx->lock_time, expiry, tx->extra_data_len, coin->curve->hasher_sign, overwintered, version_group_id);
			if (coin->decred) {
